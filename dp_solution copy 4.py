...
def traceback(dp_table, i, j):
    ...
    if left_choice > right_choice:
        my_choice = i + 1
        opp_choice = i + 2 if dp_table[i + 2][j] < dp_table[i + 1][j - 1] else j + 1
        return [my_choice, opp_choice] + traceback(dp_table, i + 2 if opp_choice == i + 2 else i + 1, j if opp_choice == i + 2 else j - 2)

    else:
        my_choice = j + 1
        opp_choice = i + 1 if dp_table[i + 1][j - 1] < dp_table[i][j - 2] else j - 1
        return [my_choice, opp_choice] + traceback(dp_table, i if opp_choice == j - 1 else i + 1, j - 2 if opp_choice == j - 1 else j - 1)
